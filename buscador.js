...
/**
 * Realiza una búsqueda en YouTube y obtiene metadatos usando Jina.ai JSON.
 * @param {string} query
 * @param {number} limit
 */
async function scrapeYoutube(query, limit = 20) {
    return withRetry(async () => {
        ...
        return data.data
            ...
            .slice(0, limit)
            .map(it => {
                const idMatch = it.url.match(/watch\?v=([\w-]{11})/);
                const videoId = idMatch ? idMatch[1] : null;
                if (!videoId) return null;

                return {
                    id: videoId,
                    title: cleanTitle(it.title || `Video ${videoId}`),
                    thumb: it.img || `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`,
                    author: cleanAuthor(it.channel || "YouTube"),
                    source: "youtube",
                    type: "youtube_video",
                    isTopic: /topic/i.test(it.channel || "")
                };
            })
            .filter(Boolean);
    });
...
/**
 * Inicializa los listeners para la búsqueda.
 */
function initSearch() {
