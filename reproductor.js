...
function savePlayerState() {
  ...
  const state = {
    ...
    wasPlaying: getPlaybackState()==="playing", timestamp: Date.now()
  };
...
function loadPlayerState() {
  ...
  try {
    ...
    return state;
  } catch (e) { console.error("Error loading player state:", e); return null; }
}
...
function updateMediaSession(track){
  ...
  if(!mediaSessionHandlersSet){
    ...
    try {
        ...
        navigator.mediaSession.setActionHandler('seekto', s(d => { if(typeof d.seekTime === 'number') seekToFrac(d.seekTime / (currentTrack.source === 'archive' ? archivePlayer.duration : ytPlayer.getDuration())); }));
