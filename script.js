...
// 16. Limpieza de metadatos (ampliado)
...
// 5. Normalización para deduplicación
const sy_normalize = str => {
    ...
    return str.toLowerCase()
        ...
        .replace(/[^\w\s-]/g, '') // Quitar caracteres no alfanuméricos (excepto guiones)
        .replace(/\s{2,}/g, ' ')
        .trim();
};

// 4. Hash estable para íconos
const sy_hashId = (id) => {
    if (!id) return 0;
    let hash = 0;
...
// 11. Debounce para guardar estado
...
let isShuffle = false;
let sy_originalQueue = null; // 12. Para shuffle estable
let sy_shuffledSeed = null;  // 12. Para shuffle estable
...
const savePlayerState = sy_debounce(() => {
  ...
  const state = {
    ...
    volume: ytPlayer.getVolume() || 100, // Persistir volumen
    isShuffle,
    sy_originalQueue, // Persistir cola original para shuffle
    sy_shuffledSeed,
    repeatMode,
...
function restorePlayerState(state) {
  ...
  const restore = () => {
    ...
    sy_originalQueue = state.sy_originalQueue || null;
    sy_shuffledSeed = state.sy_shuffledSeed || null;
...
async function fetchSpotifyPlaylist(playlistId) {
    ...
    try {
        ...
        const data = await response.json();
        return { id: data.id, name: data.name, author: data.owner.display_name, thumb: data.images?.[0]?.url || '', tracks: data.tracks.items.map(({track}) => track ? { source: 'spotify', type: 'spotify_track', id: track.id, title: track.name, author: track.artists.map(a => a.name).join(', '), thumb: track.album.images?.[0]?.url || '' } : null).filter(Boolean) };
...
async function fetchVideoDetailsByIds(ids) {
    ...
    const fetchChunk = async (chunk, retryCount = 0) => {
        ...
        try {
            ...
            if (!response.ok) { if (response.status === 403) { console.warn(`API key ${apiKey} 403 → rotando`); return fetchChunk(chunk, retryCount + 1); } throw new Error(`API error: ${response.status}`); }
            const data = await response.json();
...
function switchView(id){
  ...
  if(id!=="view-search") sy_searchCache = null; // 7. Limpiar caché al salir de la vista de búsqueda
...
searchOverlay?.addEventListener("keydown", e => { if (e.key === 'Escape') closeSearch(); }); // 17. Escape para cerrar
...
overlayInput?.addEventListener("keydown", async e=>{
    ...
    if (match && match[1]) await handleSpotifyImport(match[1]);
    else await startSearch(q);
});
...
async function youtubeSearch(query, pageToken = '', limit = BATCH_SIZE, retryCount = 0){
  ...
  const apiKey = getRotatedApiKey();
  url.searchParams.append('key', apiKey); url.searchParams.append('q', query); url.searchParams.append('part', 'snippet'); url.searchParams.append('type', 'video,playlist'); url.searchParams.append('maxResults', limit);
  if(pageToken) url.searchParams.append('pageToken', pageToken);
  try{
    const response = await fetch(url);
    if(!response.ok){ if(response.status===403){ console.warn(`API key ${apiKey} 403 → rota`); return youtubeSearch(query, pageToken, limit, retryCount+1); } throw new Error(`API error: ${response.status}`); }
    const data = await response.json();
    const resultItems = data.items.map(item => {
        // 3. Detección robusta de "Topic"
        const topicRegex = /\bTopic\b/i;
        const isTopic = topicRegex.test(item.snippet.channelTitle) && !/show|hot/i.test(item.snippet.channelTitle);
        const baseItem = {
            source: 'youtube',
...
async function startSearch(query){
  // 7. Reutilización del estado de búsqueda
  if (sy_searchCache && sy_searchCache.query === query) {
    items = sy_searchCache.items;
    paging = { query, pageToken: sy_searchCache.pageToken, loading: false, hasMore: sy_searchCache.hasMore };
  ...
  items = [];
  sy_iconMemo.clear(); // 18. Limpiar memo de íconos en nueva búsqueda
  const resultsEl = $("#results");
  ...
  try {
    ...
    items = dedupeAndSort(combined); // 5. Deduplicación y ordenamiento
    
    if (resultsEl) resultsEl.innerHTML = "";
    if (items.length === 0 && !paging.hasMore) {
...
function dedupeAndSort(arr) {
    ...
    arr.forEach(it => {
        ...
        if (!existing) {
            ...
            // Priorizar Topic, luego Spotify/mejor thumb
            if (it.isTopic && !existing.isTopic) {
                seen.set(key, it);
            } else if (!it.isTopic && !existing.isTopic) {
                if (it.source === 'spotify' && existing.source !== 'spotify') {
                    seen.set(key, it);
                } else if ((it.thumb || "").includes('high') && !(existing.thumb || "").includes('high')) {
                    seen.set(key, it);
                }
            }
        }
    ...
    // 5. Orden: Playlists -> Tracks Topic -> Tracks no Topic
    unique.sort((a, b) => {
        ...
        if (!aIsPlaylist) { // Si ambos son tracks
            if (a.isTopic !== b.isTopic) return a.isTopic ? -1 : 1;
        }
        return 0;
    });
    return unique;
...
async function loadNextPage(){
  ...
  try{
    ...
    const allItems = [...items, ...result.items];
    const dedupedItems = dedupeAndSort(allItems);
    const newItems = dedupedItems.slice(items.length);
    ...
    // 7. Actualizar caché de paginación
    if(sy_searchCache) {
      sy_searchCache.items = items;
      sy_searchCache.pageToken = paging.pageToken;
      sy_searchCache.hasMore = paging.hasMore;
    }
  }catch(e){
...
function appendResults(chunk) {
    ...
    // 9. Render sin reflows con DocumentFragment
    ...
    for (const it of chunk) {
        ...
        // 4 & 18. Lógica de íconos estable y memoizada
        if (!it.logoType) { // Calcular solo si no está definido
            if (sy_iconMemo.has(it.id)) {
                it.logoType = sy_iconMemo.get(it.id);
            } else {
                ...
                sy_iconMemo.set(it.id, it.logoType);
            }
        }
        ...
        if (it.type.includes('playlist')) {
            ...
            if (it.source === 'spotify') itemEl.classList.add("spotify-playlist-result-item");
            indicator = '<div class="playlist-indicator">LISTA</div>';
        }

        // 10. Imágenes estables
        itemEl.innerHTML = `
      <div class="thumb-wrap">
        <img class="thumb" loading="lazy" decoding="async" src="${it.thumb}" alt="" width="56" height="56">
        ${indicator}
        ${!it.type.includes('playlist') ?
          `<button class="card-play" title="Play/Pause" aria-label="Play/Pause" aria-pressed="false">
      ...
      <div class="actions">
        ...
        // 17. Micro-UX: Long-press
        let pressTimer;
        itemEl.addEventListener('touchstart', (e) => {
            if (e.target.closest(".more, .card-play")) return;
            pressTimer = setTimeout(() => {
                e.preventDefault();
                itemEl.querySelector(".more")?.click();
            }, 500);
        }, { passive: true });
    ...
    if (chunk.length > BATCH_THRESHOLD) {
        ...
        const renderBatch = () => {
            ...
            const end = Math.min(i + BATCH_RENDER_SIZE, fragment.childNodes.length);
            for (; i < end; i++) {
                batchFragment.appendChild(fragment.childNodes[0]); // Move node
            }
            root.appendChild(batchFragment);
            if (i < chunk.length) {
...
async function handleResultClick(event, item, forcePlay = false) {
    ...
    switch (item.type) {
        ...
        case 'youtube_playlist': handlePlaylistResultClick(item.id, item.title); break;
...
async function playSpotifyTrack(track) {
    ...
    resultsContainer.innerHTML = `<div class="loading-indicator" aria-live="polite"><h3>Buscando en YouTube...</h3><p>${track.author} - ${track.title}</p></div>`;
    updateHomeGridVisibility();
    ...
    } else { alert("No se pudo encontrar un video para esta canción."); }
}

// 8. Pool de promesas para concurrencia
async function sy_promisePool(poolLimit, array, iteratorFn) {
    const ret = [];
    const executing = [];
...
async function handleSpotifyImport(playlistId) {
    const resultsContainer = $("#results");
    resultsContainer.innerHTML = `<div class="loading-indicator" aria-live="polite"><h3>Importando desde Spotify...</h3><p>Esto puede tardar unos segundos.</p></div>`;
    updateHomeGridVisibility();
    try {
        ...
        // 8. Limitar concurrencia a 8
        const youtubeQueue = (await sy_promisePool(8, spotifyPlaylist.tracks, findYoutubeEquivalent)).filter(Boolean);
...
// 8. Cache para Spotify -> YT
const sy_sp2yt_cache = {
    KEY: 'sy_sp2yt_v1',
    TTL: 7 * 24 * 60 * 60 * 1000, // 7 días
    get(spId) {
        try {
            const cache = JSON.parse(localStorage.getItem(this.KEY) || '{}');
            const entry = cache[spId];
            if (entry && (Date.now() - entry.timestamp < this.TTL)) {
                return entry.ytId;
            }
            if (entry) delete cache[spId]; // Expired
    ...
    set(spId, ytId) {
        try {
            const cache = JSON.parse(localStorage.getItem(this.KEY) || '{}');
            cache[spId] = { ytId, timestamp: Date.now() };
            localStorage.setItem(this.KEY, JSON.stringify(cache));
        } catch(e) { console.error("Error guardando en caché sp2yt:", e); }
    }
};
...
async function findYoutubeEquivalent(track) {
    ...
    if (cachedYtId) {
        ...
        if(details[0]) return { ...details[0], originalId: track.id };
    }
    ...
    if (ytTrack) sy_sp2yt_cache.set(track.id, ytTrack.id);
...
async function fetchPlaylistItems(playlistId, retryCount = 0) {
    ...
    const apiKey = getRotatedApiKey(); url.searchParams.append('key', apiKey); url.searchParams.append('part', 'snippet'); url.searchParams.append('playlistId', playlistId); url.searchParams.append('maxResults', 50);
    try {
        const response = await fetch(url);
...
function renderPlaylistCard(playlist) {
    ...
    const card = document.createElement("article"); card.className = "playlist-card"; card.dataset.id = playlist.id || playlist.title;
    card.innerHTML = `<div class="collage-container">${covers.map(src => `<img src="${src}" alt="Album art collage" width="100" height="100" decoding="async">`).join('')}</div><div class="playlist-meta"><h4 class="playlist-title">${playlist.title || playlist.name}</h4><div class="creator-line">${logo}<span>Creador: ${playlist.creator}</span></div></div>`;
    card.onclick = () => {
...
function toggleFav(track){ if(isFav(track.id)) favs = favs.filter(f=>f.id!==track.id); else favs.unshift(track); saveFavs(); renderFavs(); refreshIndicators(); }
function renderFavs(){
...
async function openPlaylistOptionsMenu(pl) {
  // 14. Permisos en playlists
  if (!isMyPlaylist(pl.id)) return;

  openActionSheet({
    title: pl.name,
    actions: [ { id: "rename", label: "Renombrar" }, { id: "delete", label: "Eliminar playlist", danger: true }, { id: "cancel", label: "Cancelar", ghost: true } ],
    onAction: async (act) => {
      ...
      if (act === "rename") {
        ...
        // 15. Renombrar y "creador"
        if (newName === null || newName.trim().replace(/\s+/g, ' ') === "" || newName.trim().length > 50) {
            alert("Nombre inválido. Debe tener entre 1 y 50 caracteres."); return;
        }
        const newCreator = prompt("Nuevo nombre de creador (máx 20 caracteres):", pl.creator);
        if (newCreator === null || newCreator.trim().replace(/\s+/g, ' ') === "" || newCreator.trim().length > 20) {
            alert("Nombre de creador inválido. Debe tener entre 1 y 20 caracteres."); return;
        }
        try { await updateDoc(plRef, { name: newName.trim().replace(/\s+/g, ' '), creator: newCreator.trim().replace(/\s+/g, ' '), updatedAt: serverTimestamp() }); } catch (e) { console.error("Error al renombrar playlist:", e); alert("No se pudo renombrar la playlist."); }
      }
      if (act === "delete") {
        openActionSheet({ title: `¿Eliminar "${pl.name}"?`, actions: [ {id: "confirm_delete", label: "Sí, eliminar", danger: true}, {id: "cancel", label: "Cancelar", ghost: true} ],
            onAction: async (confirmAct) => { if(confirmAct === 'confirm_delete') { try { await deleteDoc(plRef); removeMyPlaylistId(pl.id); } catch (e) { console.error("Error al eliminar playlist:", e); alert("No se pudo eliminar la playlist."); } } }
        });
      }
    }
  });
...
function renderPlaylists() {
    const grid = $("#plList"), empty = $("#plEmpty"); if (!grid) return; grid.innerHTML = "";
    const myPlaylists = communityPlaylists.filter(p => isMyPlaylist(p.id));
    if (myPlaylists.length === 0) { empty?.classList.remove("hide"); return; } empty?.classList.add("hide");
    myPlaylists.forEach(pl => {
        const card = document.createElement("article"); card.className="pl-item"; card.dataset.plId = pl.id;
        const cover = pl.tracks[0]?.thumb || "https://i.imgur.com/gCa3j5g.png";
        card.innerHTML = `<img class="pl-thumb-bg" src="${cover}" alt="" width="150" height="180" decoding="async"><div class="pl-overlay"><div class="pl-meta"><div class="pl-title">${pl.name}</div><div class="pl-creator">por ${pl.creator || 'Anónimo'}</div><div class="pl-subtitle">${pl.tracks.length} temas</div></div><div class="pl-privacy-toggle"><label class="switch"><input type="checkbox" ${pl.isPublic ? 'checked' : ''}><span class="slider"></span></label><span>Pública</span></div></div><button class="icon-btn more" title="Opciones" aria-label="Opciones">${dotsSvg()}</button>`;
        ...
        card.addEventListener("click", (e) => { if (e.target.closest(".more") || e.target.closest('.pl-privacy-toggle')) return; showPlaylistInPlayer(pl.id); switchView("view-player"); });
...
$("#createPlConfirm").onclick = async () => {
    const name = $("#newPlName").value.trim(); const creator = $("#newPlCreator").value.trim();
    ...
    try {
        ...
        addMyPlaylistId(docRef.id); $("#newPlName").value = ""; $("#newPlCreator").value = ""; $("#createPlaylistSheet").classList.remove("show");
...
function openActionSheet({title="Opciones", actions=[], onAction=()=>{}}){
  ...
  sheet.onclick = (e)=>{ if(e.target===sheet){ sheet.classList.remove("show"); return; } const btn = e.target.closest(".sheet-item"); if(!btn) return; const id = btn.dataset.id; sheet.classList.remove("show"); if(id) onAction(id); };
}
async function openPlaylistSheet(track){
  const sheet = $("#playlistSheet"); if(!sheet) return; sheet.classList.add("show"); const list = $("#plChoices"); list.innerHTML="";
  const myPlaylists = communityPlaylists.filter(p => isMyPlaylist(p.id));
  myPlaylists.forEach(pl=>{
    const btn = document.createElement("button"); btn.className="sheet-item"; btn.textContent = pl.name;
    btn.onclick = async ()=>{
      const { doc, updateDoc, serverTimestamp } = window.firebase; const plRef = doc(db, "playlists", pl.id); const updatedTracks = [...pl.tracks];
      if (!updatedTracks.some(t => t.id === track.id)) { updatedTracks.unshift(track); }
      try { await updateDoc(plRef, { tracks: updatedTracks, updatedAt: serverTimestamp() }); sheet.classList.remove("show"); } catch(e) { console.error("Error agregando canción: ", e); alert("No se pudo agregar la canción."); }
    };
  ...
  $("#plCreateFromSong").onclick = async () => {
    ...
    try {
        ...
        addMyPlaylistId(docRef.id); $("#plNewNameFromSong").value = ""; sheet.classList.remove("show");
...
function updateUIOnTrackChange() {
  updateHero(currentTrack); updateMiniNow(); refreshIndicators(); updateControlStates(); updateMediaSession(currentTrack); updateAndroidNotification();
}
function updateHero(track){
  const t = track || currentTrack;
  const favHero = $("#favHero"); const npHero  = $("#npHero");
  ...
  let plName = "";
  if (queueType === 'playlist' && viewingPlaylistId) { const pl = communityPlaylists.find(p => p.id === viewingPlaylistId); plName = pl ? pl.name : ""; }
  else if (['recommended', 'youtube_playlist'].includes(queueType)) { plName = currentQueueTitle; }
  $("#npSub") && ($("#npSub").textContent = t ? `${cleanAuthor(t.author)}${plName ? ` • ${plName}` : ""}` : (plName || "—"));
}

function setQueue(srcArr, type, idx){
  sy_originalQueue = [...srcArr]; // Guardar siempre la original
  if (isShuffle) {
    const currentItem = srcArr[idx];
    queue = sy_fisherYatesShuffle([...srcArr], sy_shuffledSeed);
    // Mover el item actual al principio
    const newIdx = queue.findIndex(t => t.id === currentItem.id);
    if(newIdx > 0) {
...
function playCurrent(autoplay=false){
  ...
  startTimer(); updateUIOnTrackChange();
}
function playFromSearch(trackId, autoplay=false) {
    ...
    if (videoIndex > -1) { setQueue(videoItems, "search", videoIndex); viewingPlaylistId = null; playCurrent(autoplay); }
}
...
function updateMiniNow(){
  const has = !!currentTrack; const dock = $("#seekDock"); dock && dock.classList.toggle("show", has); if(!has) return;
  $("#miniThumb") && ($("#miniThumb").src = currentTrack.thumb); $("#miniTitle") && ($("#miniTitle").textContent = currentTrack.title); $("#miniAuthor") && ($("#miniAuthor").textContent = cleanAuthor(currentTrack.author) || "");
}
function getNextIndex() {
  if (!queue) return -1; if (repeatMode === 'one' && getPlaybackState() !== 'none') return qIdx;
  let next = qIdx + 1;
...
function startTimer(){
  ...
  timer = setInterval(()=>{
    ...
    $("#cur") && ($("#cur").textContent = fmt(cur)); $("#dur") && ($("#dur").textContent = fmt(dur));
    $("#seek") && ($("#seek").value = dur? Math.floor((cur/dur)*1000) : 0);
    $("#miniCur") && ($("#miniCur").textContent = fmt(cur)); $("#miniDur") && ($("#miniDur").textContent = fmt(dur));
    $("#miniSeek") && ($("#miniSeek").value = dur? Math.floor((cur/dur)*1000) : 0);
    try{ if ('mediaSession' in navigator && typeof navigator.mediaSession.setPositionState === 'function') { navigator.mediaSession.setPositionState({ duration: dur||0, playbackRate: ytPlayer.getPlaybackRate(), position: cur||0 }); } }catch(e) {}
    savePlayerState();
  }, 500);
}
...
/* ========= Shuffle / Repeat ========= */
// 12. Shuffle estable (Fisher-Yates con semilla)
function sy_fisherYatesShuffle(array, seed) {
...
function toggleShuffle() {
  ...
  if (isShuffle) {
    sy_shuffledSeed = Date.now();
    if(sy_originalQueue) { // Si ya hay una cola, la barajamos
        setQueue(sy_originalQueue, queueType, qIdx);
    }
  } else {
    sy_shuffledSeed = null;
    if(sy_originalQueue) { // Volver a la cola original
        const currentId = currentTrack?.id;
        queue = [...sy_originalQueue];
        qIdx = currentId ? queue.findIndex(t => t.id === currentId) : -1;
    }
  }
...
function cycleRepeat() {
  const modes = ['none', 'all', 'one']; const currentModeIdx = modes.indexOf(repeatMode); repeatMode = modes[(currentModeIdx + 1) % modes.length];
  const btn = $("#btnRepeat");
...
function renderQueue(queueItems, title) {
    const panel = $("#queuePanel"); currentQueueTitle = title; panel && panel.classList.remove("hide");
    const header = panel.querySelector(".section-head");
    if (header) { let saveBtn = header.querySelector('#btnSavePlaylist'); if (saveBtn) saveBtn.remove(); const titleEl = header.querySelector('#queueTitle'); if (titleEl) titleEl.textContent = title; if ((queueType === 'youtube_playlist' || queueType === 'spotify_playlist') && queue?.length > 0) { saveBtn = document.createElement('button'); saveBtn.id = 'btnSavePlaylist'; saveBtn.className = 'pill'; saveBtn.textContent = 'Guardar Lista'; saveBtn.onclick = saveCurrentQueueAsPlaylist; header.appendChild(saveBtn); } }
    const ul = $("#queueList"); if (!ul) return; ul.innerHTML = "";
    const isUserPlaylist = queueType === 'playlist'; if (!isUserPlaylist) viewingPlaylistId = null;
    (queueItems || []).forEach((t, i) => {
        ...
        li.onclick = (e) => { if (e.target.closest(".more") || e.target.closest(".card-play")) return; qIdx = i; playCurrent(true); };
        li.querySelector(".card-play").onclick = (e) => { e.stopPropagation(); qIdx = i; playCurrent(true); };
        ul.appendChild(li);
    });
    refreshIndicators();
}
async function saveCurrentQueueAsPlaylist() {
    ...
    if (!creator) { creator = prompt("Para guardar, ingresá tu nombre de creador:")?.trim(); if (!creator) return; localStorage.setItem('sy_creator_name', creator); }
    const btn = $('#btnSavePlaylist'); if (btn) { btn.disabled = true; btn.textContent = 'Guardando...'; }
    try {
        const { collection, addDoc, serverTimestamp } = window.firebase;
        ...
        addMyPlaylistId(docRef.id); if (btn) { btn.textContent = 'Guardada ✔'; }
    } catch (e) { console.error("Error guardando la playlist: ", e); alert("Hubo un error al guardar la playlist."); if (btn) { btn.disabled = false; btn.textContent = 'Guardar Lista'; } }
}
...
document.addEventListener("click", async (e) => {
    ...
    const itemEl = btn.closest(".result-item, .fav-item, .queue-item"); if (!itemEl) return;
    let track; const trackId = itemEl.dataset.trackId;
    if (itemEl.classList.contains("result-item")) {
        const itemId = itemEl.dataset.itemId; const sourceTrack = items.find(x => x.id === itemId); if (!sourceTrack) return;
        if (sourceTrack.type.includes('playlist')) return;
        if (sourceTrack.type === 'spotify_track') { const ytEquivalent = await findYoutubeEquivalent(sourceTrack); if (!ytEquivalent) { alert("No se pudo encontrar esta canción en YouTube para agregarla."); return; } track = ytEquivalent; } else { track = sourceTrack; }
    } else if (itemEl.classList.contains("fav-item")) { track = favs.find(f => f.id === trackId);
    } else if (itemEl.classList.contains("queue-item")) { const trackIndex = Array.from(itemEl.parentNode.children).indexOf(itemEl); track = queue[trackIndex]; }
    if (!track) return;
    const actions = [ { id: "fav", label: isFav(track.id) ? "Quitar de Favoritos" : "Agregar a Favoritos" }, { id: "pl", label: "Agregar a playlist" } ];
    // 14. Permisos en playlists
    if (itemEl.classList.contains("queue-item") && queueType === 'playlist' && viewingPlaylistId && isMyPlaylist(viewingPlaylistId)) { actions.push({ id: "delete", label: "Eliminar de esta playlist", danger: true }); }
    actions.push({ id: "cancel", label: "Cancelar", ghost: true });
    openActionSheet({ title: track.title, actions: actions, onAction: (act) => { if (act === "fav") toggleFav(track); if (act === "pl") openPlaylistSheet(track); if (act === "delete") { removeFromPlaylist(viewingPlaylistId, track.id); } } });
});
...
function refreshIndicators(){
  ...
  $$(".result-item, .fav-item, .queue-item").forEach(el => {
    let trackId = el.dataset.trackId; const isCurrentTrack = trackId === curId;
    el.classList.toggle("is-playing", isCurrentTrack);
    const cardPlay = el.querySelector(".card-play");
    // 17. Sincronizar aria-pressed
    if (cardPlay) { cardPlay.classList.toggle("playing", isPlaying && isCurrentTrack); cardPlay.setAttribute("aria-pressed", isPlaying && isCurrentTrack); }
  });
  $("#npPlay")?.classList.toggle("playing", isPlaying); $("#miniPlay")?.classList.toggle("playing", isPlaying);
}
...
window.onYouTubeIframeAPIReady = function(){
  ytPlayer = new YT.Player("player",{ width:300, height:150, videoId:"", playerVars:{autoplay:0, controls:0, rel:0, playsinline:1},
    events:{
      ...
      onStateChange:(e)=>{
        const st = e.data; if(st===YT.PlayerState.ENDED){ next(); }
        // 13. Verificación de mediaSession
        if('mediaSession' in navigator){ navigator.mediaSession.playbackState = (st===YT.PlayerState.PLAYING || st===YT.PlayerState.BUFFERING) ? 'playing' : (st===YT.PlayerState.PAUSED ? 'paused' : 'none'); }
        refreshIndicators(); updateAndroidNotification();
      },
      // 11. Manejo de errores del reproductor
      onError: (e) => {
        console.error(`Error de YouTube Player (${e.data}): saltando a la siguiente canción.`);
        next();
      }
    }
  });
...
/* ========= 6. Scroll infinito con IntersectionObserver ========= */
const sentinel = $("#sentinel");
if (sentinel){
  const io = new IntersectionObserver((entries)=>{
    if (entries[0].isIntersecting && paging.hasMore && !paging.loading){ loadNextPage(); }
...
function updateMediaSession(track){
  ...
  if(!mediaSessionHandlersSet){ // 13. Registrar handlers
    ...
    navigator.mediaSession.setActionHandler('seekbackward',s(d=>{ const o=d.seekOffset||10; if(!YT_READY)return; ytPlayer.seekTo(Math.max(0,(ytPlayer.getCurrentTime()||0)-o),true) }));
    navigator.mediaSession.setActionHandler('seekforward',s(d=>{ const o=d.seekOffset||10; if(!YT_READY)return; ytPlayer.seekTo((ytPlayer.getCurrentTime()||0)+o,true) }));
    navigator.mediaSession.setActionHandler('seekto',s(d=>{ if(!YT_READY||!d||typeof d.seekTime!=='number')return; ytPlayer.seekTo(d.seekTime,true) }));
  }
  const st=getPlaybackState(); navigator.mediaSession.playbackState=(st==='playing'?'playing':(st==='paused'?'paused':'none'));
}
