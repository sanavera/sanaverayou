...
function savePlayerState(){if(!currentTrack||!ytPlayer)return;const s={queue,queueType,qIdx,currentTime:ytPlayer.getCurrentTime()||0,isShuffle,repeatMode,wasPlaying:getPlaybackState()==="playing",timestamp:Date.now()};try{localStorage.setItem(PLAYER_STATE_KEY,JSON.stringify(s))}catch(e){console.error("Error al guardar estado:",e)}}
function loadPlayerState(){const s=localStorage.getItem(PLAYER_STATE_KEY);if(!s)return null;try{const t=JSON.parse(s);if(Date.now()-(t.timestamp||0)>72e5){localStorage.removeItem(PLAYER_STATE_KEY);return null}return t}catch(e){console.error("Error al cargar estado:",e);return null}}
...
async function startSearch(s){searchAbort&&searchAbort.abort(),searchAbort=new AbortController,items=[];const t=$("#results");t&&(t.innerHTML='<div class="loading-indicator"><h3>Buscando...</h3></div>'),updateHomeGridVisibility();try{const[e,o]=await Promise.all([youtubeSearch(s,20),searchSpotify(s,20)]);if(searchAbort.signal.aborted)return;let a=[...o.playlists,...e.filter(l=>l.type==="youtube_playlist"),...o.tracks,...e.filter(l=>l.type==="youtube_video")];a.sort((l,n)=>{const c=l.type.includes("playlist"),d=n.type.includes("playlist");return c&&!d?-1:!c&&d?1:0}),t&&(t.innerHTML=""),a.length===0?(t&&(t.innerHTML='<div class="loading-indicator"><p>No se encontraron resultados.</p></div>'),void 0):(items=dedupeById(a),appendResults(items))}catch(e){console.error("Search failed:",e),t&&(t.innerHTML='<div class="loading-indicator"><p>Error en la búsqueda.</p></div>')}}
...
async function handleResultClick(s,t,e=!1){if(s.target.closest(".more")||s.target.closest(".card-play")&&!e)return;switch(t.type){case"youtube_video":playFromSearch(t.id,!0);break;case"spotify_track":await playSpotifyTrack(t);break;case"spotify_playlist":await handleSpotifyImport(t.id);break}}
...
/* ========= Lógica de Importación de Playlists (NUEVO) ========= */
function showProgressModal(s,t){hideProgressModal();const e=$("#queuePanel");if(!e)return;const o=document.createElement("div");o.id="importProgressModal",o.className="import-progress",o.innerHTML=`<h3>${s}</h3><p>${t||"Por favor, espera..."}</p>`,e.prepend(o)}
...
async function handleSpotifyImport(s){const t=await fetchSpotifyPlaylist(s);if(!t||!t.tracks.length)return alert("No se pudo cargar la playlist de Spotify o está vacía.");switchView("view-player"),renderQueue([],t.name),showProgressModal("Importando Playlist de Spotify",`Buscando ${t.tracks.length} canciones...`);const e=[],o=t.tracks.length;for(let a=0;a<o;a++){const i=t.tracks[a];updateProgressModal(`Importando ${a+1}/${o}: "${i.title}"`);const r=await findYoutubeEquivalent(i);r&&(e.push(r),setQueue(e,"spotify_playlist",qIdx),renderQueue(e,t.name)),await new Promise(l=>setTimeout(l,150))}hideProgressModal(),setQueue(e,"spotify_playlist",0),renderQueue(e,t.name),playCurrent(!0)}
...
async function showPlaylistInPlayer(s){const t=communityPlaylists.find(r=>r.id===s);if(!t)return;let e=t.tracks;if(t.source==="spotify"&&(!t.tracks||t.tracks.length===0)){switchView("view-player"),renderQueue([],t.name),showProgressModal(`Convirtiendo "${t.name}"`,"Buscando canciones en YouTube, por favor espera...");try{const r=t.spotifyTracks||[];if(r.length>0){const l=[];for(let n=0;n<r.length;n++){const c=r[n];updateProgressModal(`Buscando ${n+1}/${r.length}: "${c.title}"`);const d=await findYoutubeEquivalent(c);d&&l.push(d),await new Promise(u=>setTimeout(u,150))}if(l.length>0){const{doc:n,updateDoc:c}=window.firebase;await c(n(db,"playlists",t.id),{tracks:l}),e=l}else throw new Error("No se encontraron equivalentes en YouTube para esta lista.")}else throw new Error("La lista de Spotify no tiene canciones para buscar.")}catch(r){return alert(`Error: ${r.message}`),console.error(r),void switchView("view-playlists")}finally{hideProgressModal()}}if(!e||e.length===0)return void alert(`La playlist "${t.name}" está vacía o no se pudieron encontrar las canciones.`);viewingPlaylistId=t.id,setQueue(e,"playlist",0),renderQueue(e,t.name),playCurrent(!0)}
...
window.handleNativeControl=function(s){const t=String(s||"").toLowerCase();t==="action_play"?YT_READY&&ytPlayer&&ytPlayer.playVideo():t==="action_pause"?YT_READY&&ytPlayer&&ytPlayer.pauseVideo():t==="action_next"?next():t==="action_prev"&&prev()};async function boot(){initTheme();const s={apiKey:"AIzaSyBojG3XoEmxcxWhpiOkL8k8EvoxIeZdFrU",authDomain:"sanaverayou.firebaseapp.com",projectId:"sanaverayou",storageBucket:"sanaverayou.appspot.com",messagingSenderId:"275513302327",appId:"1:275513302327:web:3b26052bf02e657d450eb2"};const{initializeApp:t}=await import("https://www.gstatic.com/firebasejs/9.6.1/firebase-app.js"),{getFirestore:e,collection:o,onSnapshot:a,query:i,orderBy:r,doc:l,updateDoc:n,addDoc:c,serverTimestamp:d,deleteDoc:u}=await import("https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js");window.firebase={getFirestore:e,collection:o,onSnapshot:a,query:i,orderBy:r,doc:l,updateDoc:n,addDoc:c,serverTimestamp:d,deleteDoc:u};const m=t(s);db=e(m),a(i(o(db,"playlists"),r("updatedAt","desc")),p=>{communityPlaylists=p.docs.map(g=>({id:g.id,...g.data()})),renderPlaylists(),renderAllHomePlaylists(),viewingPlaylistId&&queueType==="playlist"&&(()=>{const g=communityPlaylists.find(f=>f.id===viewingPlaylistId);if(g){const f=currentTrack?currentTrack.id:null;renderQueue(g.tracks,g.name),setQueue(g.tracks,"playlist",qIdx);const k=g.tracks.findIndex(y=>y.id===f);k!==-1?qIdx=k:(qIdx=Math.min(qIdx,g.tracks.length-1),g.tracks.length===0?(currentTrack=null,ytPlayer.stopVideo()):currentTrack=queue[qIdx],updateUIOnTrackChange())}else hideQueuePanel(),queueType==="playlist"&&(currentTrack=null,queue=null,ytPlayer.stopVideo(),updateUIOnTrackChange())})()});const p=Object.keys(recommendedPlaylists),g=await Promise.all(p.map(k=>fetchVideoDetailsByIds(recommendedPlaylists[k].ids)));p.forEach((k,y)=>{recommendedPlaylists[k].data=g[y]||[]}),renderAllHomePlaylists(),updateHomeGridVisibility(),loadFavs(),renderFavs(),loadYTApi();const f=loadPlayerState();f&&restorePlayerState(f),heroScrollInvalidate(),document.title="SanaveraYou Pro"}
function renderAllHomePlaylists(){const s=$("#allPlaylistsContainer");s&&(s.innerHTML="");const t=communityPlaylists.filter(o=>o.isPublic&&(o.tracks?.length>0||o.spotifyTracks?.length>0)),e=[...Object.values(recommendedPlaylists).filter(o=>o.data.length>0),...t];e.sort((o,a)=>{const i=o.updatedAt?.toDate()||new Date(0),r=a.updatedAt?.toDate()||new Date(0);return r-i});for(const o of e)renderPlaylistCard(o)}
boot(),window.addEventListener("beforeunload",savePlayerState),window.addEventListener("beforeunload",function(){canUseAndroidBridge()&&AndroidBridge.stopNotification()});

```json
{"user_input": "Ok ahora funciona busca música correctamente ahora vamos hacer una cosa presta atención lo que vamos hacer. Te voy a pasar un script que tiene una lógica de importar listas de reproducción de usuarios de Spotify pero todo lo que se basaba en usar la api de YouTube la vamos a remplazar por nuestro sistema de scraping que hicimos recién para probar. Ok? Si un usuario busca música va a buscar música y listas de reproducción en Spotify. Cuando esten en los resultados si entramos a una lista de reproducción de Spotify vamos a scrapping a las canciones en la pestaña de reproductor. Lo mismo en la pestaña de Playlist que hay para importar listas de reproducción de usuarios. Traemos todas las listas (eso ya esta) y una vez guardadas si el usuario toca una lista vamos a scrapping a las canciones es decir buscar una por una después esos datos los guardamos para que si se vuelve a acceder a esa lista ya no haga falta scrapping. Es decir en resumen todo lo que hacía la api de YouTube ahora lo hacemos con scrapping con el metodo que hicimos recién. Si scrapping a una lista de reproducción informar al usuario que debe esperar mostrar un mini modal del progreso ejemplo\"Importando cancion 1/15\" en la pestaña reproductor las que ya estén lista ya se podrán reproducir. Hace todo eso y pasame completo por favor. Gracias", "files": ["uploaded:script.js"]}
