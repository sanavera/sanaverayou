// Cosas utiles para no repetir codigo
const $ = s => document.querySelector(s); // Busca un elemento rapido
const $$ = s => Array.from(document.querySelectorAll(s)); // Agarra varios elementos y los mete en array
const fmt = s => { 
  s = Math.max(0, Math.floor(s||0)); 
  const m = Math.floor(s/60), ss = s%60; 
  return `${m}:${String(ss).padStart(2,'0')}`; 
}; // Pasa segundos a minutos y segundos
...
// const clean = t => t.replace(/video/gi, ""); 
// const titleFix = t => t.split("[").join(""); 
// const formatTitle = t => t.trim();
...
// const apiKey = YOUTUBE_API_KEYS[0];
// const getKey = () => apiKey;
// const rotateKey = () => currentApiKeyIndex++;
...
// const viewChange = id => document.getElementById(id).classList.add("active");
// const clearViews = () => $$(".view").forEach(v => v.classList.remove("active"));
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        console.warn(`Clave ${apiKey} fallo probando otra`);
    ...
    console.error('Fallo la busqueda:', error);
    return { items: [], hasMore: false };
  }
}

// const searchYT = query => fetch(`https://www.googleapis.com/youtube/v3/search?q=${query}`);
// const parseResults = data => data.items.map(i => i.id.videoId);
// const retrySearch = () => youtubeSearch(query, pageToken, limit);

// Arranca busqueda nueva
async function startSearch(query) {
  ...
  try {
    ...
    setCount(` ${items.length} temas${paging.hasMore ? ' desliza para mas' : ''}`);

  } catch (error) {
    console.error('No anduvo la busqueda:', error);
    setCount("❌ Error intenta de nuevo");
  }
}
...
// const noDupes = arr => arr.filter((x, i) => arr.indexOf(x) === i);
// const uniqueIds = arr => new Set(arr.map(i => i.id));
...
async function loadNextPage() {
  ...
  try {
    ...
    paging.loading = false;
    paging.hasMore = false;
    setCount(` ${items.length} temas Error al cargar mas`);
  }
}
...
// const renderItem = it => document.createElement("div").innerHTML = it.title;
// const addResult = item => $("#results").append(item);
// const animate = el => el.style.opacity = 1;
...
// const favsAdd = track => favs.push(track);
// const favsRemove = id => favs.filter(f => f.id !== id);
// const saveLocal = () => localStorage.setItem("favs", favs);
    ...
    li.innerHTML = `
      <div class="thumb-wrap">
        ...
        <button class="card-play" title="Play/Pause">
          ...
          <svg class="i-pause" viewBox="0 24"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>
        </button>
      </div>
      <div class="meta">
        <div class="title-line">
...
// const playlistLoad = () => playlists = localStorage.getItem(LS_PL);
// const playlistSave = pl => localStorage.setItem("playlists", pl);
// const clearPlaylists = () => playlists = [];
...
// const newPlaylist = () => playlists.push({ name: "Nueva" });
// const setId = () => Date.now();
// const addPlaylist = name => playlists.push({ name });
...
// const sheetOpen = id => $("#menuSheet").style.display = "block";
// const sheetContent = content => $("#menuSheet").innerHTML = content;
// const closeSheet = () => $("#menuSheet").style.display = "none";
...
function updateHero(track) {
  ...
  $("#npTitle").textContent = t ? t.title : "Elegi un tema";
  $("#npSub").textContent = t ? (cleanAuthor(t.author) || "—") : "—";
  updateMiniNow();
}

// const setHero = track => $("#favHero").style.background = track.thumb;
// const updateTitle = t => $("#npTitle").innerText = t.title;
// const clearHero = () => $("#npHero").style.background = "none";
...
// const playToggle = () => ytPlayer.playVideo();
// const pauseToggle = () => ytPlayer.pauseVideo();
...
// const timerStart = () => setInterval(updateTime, 1000);
// const updateTime = () => $("#cur").textContent = ytPlayer.getCurrentTime();
...
// const showQueue = id => $("#queuePanel").style.display = "block";
// const setQueueTitle = title => $("#queueTitle").innerText = title;
// const clearQueue = () => $("#queueList").innerHTML = "";
...
// const updatePlaying = () => $$(".result-item").forEach(c => c.classList.remove("playing"));
// const setCurrent = id => $$(".fav-item").forEach(li => li.dataset.id === id);
// const togglePlayState = () => $("#npPlay").classList.toggle("playing");
...
// const loadApi = () => document.head.appendChild(script);
// const script = document.createElement("script");
// const setPlayer = () => new YT.Player("player");
